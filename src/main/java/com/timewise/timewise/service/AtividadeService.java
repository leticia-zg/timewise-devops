...
public class AtividadeService {
    ...
    public AtividadeResponseDTO criar(AtividadeRequestDTO atividadeDTO) {
        ...
        tentarRecalcularScore(atividadeSalva.getUsuario().getId(), dataAtividade);
    ...
    public AtividadeResponseDTO atualizar(Long id, AtividadeRequestDTO atividadeDTO) {
        ...
        tentarRecalcularScore(atividadeSalva.getUsuario().getId(), dataNova);
        if (dataAntiga != null && !dataAntiga.equals(dataNova)) {
            tentarRecalcularScore(atividadeSalva.getUsuario().getId(), dataAntiga);
        }
    ...
    public void deletar(Long id) {
        ...
        // Recalcula score imediatamente após remover a atividade
        if (usuarioId != null && dataAtividade != null) {
            tentarRecalcularScore(usuarioId, dataAtividade);
        }
    }
    ...
    /**
     * Recalcula o score diário para um usuário em uma data específica
     * @param usuarioId - ID do usuário
     * @param dataTrabalho - Data da atividade
     */
    private void tentarRecalcularScore(Long usuarioId, LocalDate dataTrabalho) {
        if (usuarioId == null || dataTrabalho == null) {
            log.warn("Dados insuficientes para recalcular score diário (usuarioId: {}, dataTrabalho: {})", 
                usuarioId, dataTrabalho);
            return;
        }
        try {
            ScoreDiario scoreRecalculado = scoreDiarioService.calcularESalvarScore(usuarioId, dataTrabalho);
            if (scoreRecalculado != null) {
                log.info("Score diário recalculado para usuário ID: {} na data: {} - valor: {}", 
                    usuarioId, dataTrabalho, scoreRecalculado.getValor());
            }
        } catch (Exception exception) {
            log.error("Erro ao recalcular score diário (usuarioId: {}, dataTrabalho: {}) - {}", 
                usuarioId, dataTrabalho, exception.getMessage(), exception);
        }
    }
}
